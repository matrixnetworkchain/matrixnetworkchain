...
//
// Bootup the masternode, look for a 10000000 MAT input and register on the network
//
void CActiveMasternode::ManageStatus()
{
    ...
    if(status == MASTERNODE_NOT_PROCESSED) {
        ...
        if(GetMasterNodeVin(vin, pubKeyCollateralAddress, keyCollateralAddress)) {
            ...
            /* donations are not supported in matrixnetwork.conf */
...
{
    CMATAddress address(collateralAddress);
