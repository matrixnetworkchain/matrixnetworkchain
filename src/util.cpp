...
#else
    const char* pszModule = "MatrixNetwork";
#endif
    if (pex)
        return strprintf(
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\MatrixNetwork
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\MatrixNetwork
    // Mac: ~/Library/Application Support/MatrixNetwork
    // Unix: ~/.MatrixNetwork
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "MatrixNetwork";
#else
    fs::path pathRet;
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "matrixnetwork.conf"));
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty matrixnetwork.conf if it does not exist
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "matrixnetworkd.pid"));
