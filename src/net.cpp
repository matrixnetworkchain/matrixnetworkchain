...
void CNode::copyStats(CNodeStats &stats)
{
    ...
    // due to a new block arriving or other large MatrixNetwork.
...
void ThreadMapPort()
{
    ...
    if (r == 1)
    {
        ...
        string strDesc = "MatrixNetwork " + FormatFullVersion();
...
bool BindListenPort(const CService &addrBind, string& strError)
{
    ...
    if (::bind(hListenSocket, (struct sockaddr*)&sockaddr, len) == SOCKET_ERROR)
    {
        ...
        if (nErr == WSAEADDRINUSE)
            strError = strprintf(_("Unable to bind to %s on this computer. MatrixNetwork is probably already running."), addrBind.ToString());
        else
