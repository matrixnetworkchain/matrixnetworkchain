...
void OverviewPage::updateDarksendProgress()
{
    ...
    QString strAnonymizeMATAmount = BitcoinUnits::formatHtmlWithUnit(nDisplayUnit, nAnonymizeMATAmount * COIN, false, BitcoinUnits::separatorAlways);
...
//        strAnonymizeMATAmount = strAnonymizeMATAmount.remove(strAnonymizeMATAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAnonymizeMATAmount = strAnonymizeMATAmount.remove(strAnonymizeMATAmount.indexOf("."), 2 + 1);
        strAmountAndRounds = strAnonymizeMATAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    ...
    if(nMaxToAnonymize > nAnonymizeMATAmount*COIN) nMaxToAnonymize = nAnonymizeMATAmount*COIN;

    if(nMaxToAnonymize == 0) return;

    if(nMaxToAnonymize >= nAnonymizeMATAmount * COIN) {
        ui->labelAmountRounds->setToolTip(tr("Found enough compatible inputs to anonymize %1")
                                          .arg(strAnonymizeMATAmount));
//        strAnonymizeMATAmount = strAnonymizeMATAmount.remove(strAnonymizeMATAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAnonymizeMATAmount = strAnonymizeMATAmount.remove(strAnonymizeMATAmount.indexOf("."), 2 + 1);
        strAmountAndRounds = strAnonymizeMATAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    } else {
...
//        strMaxToAnonymize = strMaxToAnonymize.remove(strMaxToAnonymize.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strMaxToAnonymize = strMaxToAnonymize.remove(strMaxToAnonymize.indexOf("."), 2 + 1);
