...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nAnonymizeMATAmount"))
        settings.setValue("nAnonymizeMATAmount", 1000);
    nAnonymizeMATAmount = settings.value("nAnonymizeMATAmount").toLongLong();
    if (settings.contains("nDarksendRounds"))
        SoftSetArg("-darksendrounds", settings.value("nDarksendRounds").toString().toStdString());
    if (settings.contains("nAnonymizeMATAmount"))
        SoftSetArg("-anonymizematamount", settings.value("nAnonymizeMATAmount").toString().toStdString());
    ...
    if (!language.isEmpty())
    if (!SoftSetArg("-lang", language.toStdString()))
...
QVariant OptionsModel::data(const QModelIndex & index, int role) const
{
    if(role == Qt::EditRole)
    {
        ...
        case AnonymizeMATAmount:
            return QVariant(nAnonymizeMATAmount);
        case UseBlackTheme:
...
bool OptionsModel::setData(const QModelIndex & index, const QVariant & value, int role)
{
    ...
    if(role == Qt::EditRole)
    {
        ...
        case DarksendRounds:
            ...
            break;
        case AnonymizeMATAmount:
            nAnonymizeMATAmount = value.toInt();
            settings.setValue("nAnonymizeMATAmount", nAnonymizeMATAmount);
            emit AnonymizeMATAmountChanged(nAnonymizeMATAmount);
            break;
        default:
            break;
        }
