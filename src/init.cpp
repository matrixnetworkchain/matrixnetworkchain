...
void Shutdown()
{
    ...
    RenameThread("MatrixNetwork-shutoff");
...
std::string HelpMessage()
{
    ...
    strUsage += "  -conf=<file>           " + _("Specify configuration file (default: matrixnetwork.conf)") + "\n";
    strUsage += "  -pid=<file>            " + _("Specify pid file (default: matrixnetworkd.pid)") + "\n";
    ...
    strUsage += "  -port=<port>           " + _("Listen for connections on <port> (default: 22616)") + "\n";
    ...
    strUsage += "  -rpcport=<port>        " + _("Listen for JSON-RPC connections on <port> (default: 22617)") + "\n";
...
strUsage += "\n" + _("Masternode options:") + "\n";
    ...
    strUsage += "  -anonymizematamount=<n> " + _("Keep N MAT anonymized (default: 0)") + "\n";
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. MatrixNetwork is shutting down."));
    ...
    if (!lock.try_lock())
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. MatrixNetwork is probably already running."), strDataDir));

    if (GetBoolArg("-shrinkdebugfile", !fDebug))
        ShrinkDebugFile();
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("MatrixNetwork version %s (%s)\n", FormatFullVersion(), CLIENT_DATE);
    ...
    if (fDaemon)
        fprintf(stdout, "MatrixNetwork server starting\n");
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK)
        {
            ...
            else if (nLoadWalletRet == DB_NONCRITICAL_ERROR)
            {
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of MatrixNetwork") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE)
            {
                strErrors << _("Wallet needed to be rewritten: restart MatrixNetwork to complete") << "\n";
    ...
    nAnonymizeMATAmount = GetArg("-anonymizematamount", 0);
    if(nAnonymizeMATAmount > 999999) nAnonymizeMATAmount = 999999;
    if(nAnonymizeMATAmount < 2) nAnonymizeMATAmount = 2;
    ...
    LogPrintf("Anonymize MatrixNetwork Amount %d\n", nAnonymizeMATAmount);

    /* Denominations
       A note about convertability. Within Darksend pools, each denomination
       is convertable to another.
       For example:
       1MAT+1000 == (.1MAT+100)*10
       10MAT+10000 == (1MAT+1000)*10
    */
